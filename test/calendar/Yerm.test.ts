import { INVALID_DAY, INVALID_MONTH, INVALID_YERM } from "../../src/Const";
import { YermCalendar as cal } from "../../src/calendar/YermCalendar";
...
describe("yerm calendar spec", () => {
  it("should convert an Yerm date to Julian day number (JDN)", () => {
    dates.forEach(({ jdn, date }) =>
      expect(cal.toJdn(date.cycle, date.yerm, date.month, date.day)).toBe(jdn),
    );
  });
