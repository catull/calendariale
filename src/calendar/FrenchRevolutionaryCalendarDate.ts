...
export class FrenchRevolutionaryCalendarDate extends YearMonthCalendarDate {
  ...
  public static validate(an: number, mois: number, decade: number, jour: number): void {
    ...
    if (decade < 1 || decade > 3) {
      throw new CalendarDateValidationException(INVALID_DECADI);
    }

    const sansCullotides: number = this.isLeapYear(an) ? 6 : 5;

    if (mois === 0 && jour > sansCullotides) {
      throw new CalendarDateValidationException(INVALID_JOUR);
    }
  ...
  private static anneeDeLaRevolution(jdn: number): number[] {
    let guess = GregorianCalendarDate.fromJdn(jdn).getYear() - 2;
