...
export class HinduLunarModernCalendar {
  ...
  private static calculateJdn(
    ...
    const approx: number =
      hindu.EPOCH_RD + hindu.SIDEREAL_YEAR * (year + hindu.LUNAR_ERA + (month - 1) / 12);
    const s: number = Math.floor(
      approx -
        hindu.SIDEREAL_YEAR * mod3(hinduSolarLongitude(approx) / 360 - (month - 1) / 12, -0.5, 0.5),
    );
    ...
    if (k > 3 && k < 27) {
      ...
      temp =
        mid.getMonth() !== month || (mid.isMonthLeap() && !monthLeap)
          ? mod3(k, -15, 15)
          : mod3(k, 15, 45);
    }
