...
export class RomanCalendar {
   ...
   * @returns either the 15th or 13th
   */
  ...
  private static validate(
    ...
    const maxKalends: number =
      ROMAN_MONTH_MAX_DAYS[mod(month - 2, 12)] - this.idesOfMonth(previousMonth) + 1;
    const maxCount: number =
      event === RomanEvent.IDES
        ? 8
        : event === RomanEvent.NONES
          ? this.nonesOfMonth(month) - 1
          : maxKalends;
