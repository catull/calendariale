...
export class BahaiCalendarDate extends LeapCalendarDate {
  ...
  public static bahaiToJdn(kullIshay: number, vahid: number, year: number, month: number, day: number) {
    ...
    const gy: number = byear + GregorianCalendarDate.fromJdn(bahai.EPOCH).getYear() - 1;
    ...
    if (byear < 172) {
      leap = GregorianCalendarDate.isLeapYear(gy + 1);
      jd = GregorianCalendarDate.toJdn(gy, 3, 20);
    } else {
  ...
  public static isLeapYear(year: number): boolean {
    ...
    if (gy < 2015) {
      return GregorianCalendarDate.isLeapYear(gy);
    }
  ...
  public static fromJdn(jdn: number): BahaiCalendarDate {
    ...
    if (old) {
      const gy: number = GregorianCalendarDate.fromJdn(jd0).getYear();
      leap = GregorianCalendarDate.isLeapYear(gy + 1);
      const bstarty: number = GregorianCalendarDate.fromJdn(bahai.EPOCH).getYear();
      bys = gy - (bstarty + (GregorianCalendarDate.toJdn(gy, 1, 1) <= jd0 &&
        jd0 <= GregorianCalendarDate.toJdn(gy, 3, 20) ? 1 : 0)) + 1;
      by = [0, 0];
    } else {
  ...
  private static lastTehranEquinox(jd: number, epoch: number): number[] {
    let guess: number = GregorianCalendarDate.fromJdn(jd).getYear() - 2;
  ...
  constructor(
    ...
    month: number,
    day: number) {
    super(jdn, year, month, day, BahaiCalendarDate.isLeapYear(year));
  }
