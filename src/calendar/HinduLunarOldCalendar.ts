...
export class HinduLunarOldCalendar extends Calendar {
  constructor (year: number, month: number, protected monthLeap: boolean, day: number) {
    super (year, month, day);

     this.jdn = HinduLunarOldCalendar.toJdn (year, month, monthLeap, day);
     this.yearLeap = HinduLunarOldCalendar.isLeapYear (year);
  }
  ...
  public static fromJdn (jdn: number): Calendar {
    ...
    const monthLeap = ARYA_SOLAR_MONTH - ARYA_LUNAR_MONTH >=
                        mod (newMoon, ARYA_SOLAR_MONTH) &&
    ...
    return new HinduLunarOldCalendar (year, month, monthLeap, day);
  }
}
