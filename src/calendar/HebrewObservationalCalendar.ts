...
export class HebrewObservationalCalendar {
  ...
  public static fromJdn(jdn: number): HebrewObservationalDate {
    ...
    const year: number =
      HebrewCalendar.fromJdn(newYear2).getYear() + (month >= HebrewMonth.TISHRI ? 1 : 0);
  ...
  private static toNewYear(year: number): number {
    ...
    const sset: number = standardToUniversal(
      sunset(Math.floor(equinox), hebrew.LOCATION_JAFFA),
      hebrew.LOCATION_JAFFA,
    );
