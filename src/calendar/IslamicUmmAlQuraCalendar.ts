...
export class IslamicUmmAlQuraCalendar {
  ...
  public static toJdn(year: number, month: number, day: number): number {
    ...
    const midmonth =
      islamic.EPOCH + Math.floor(((year - 1) * 12 + month - 0.5) * MEAN_SYNODIC_MONTH);

    return J0000 + this.saudiNewMonthOnOrBefore(midmonth) + day - 1;
  }

  // Is a given year in the Umm al-Qura calendar a leap year (355 days)?
  ...
  private static saudiCriterion(rataDie: number): boolean {
    ...
    return (
      ...
      moonLag(rataDie - 1, islamic.LOCATION_MECCA) > 0
    );
  }
}
