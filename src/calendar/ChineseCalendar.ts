...
export class ChineseCalendar {
  ...
  public static fromRd(rataDie: number): ChineseDate {
    ...
    const month: number = amod(
      Math.round((m - m12) / MEAN_SYNODIC_MONTH) -
        (yearLeap && this.isChinesePriorLeapMonth(m12, m) ? 1 : 0),
      12,
    );
    ...
    const years: number = Math.floor(
      1.5 - month / 12 + (rataDie - this.getEpochRD()) / MEAN_TROPICAL_YEAR,
    );
  ...
  protected static getLocation(rataDie: number): Location {
    return rataDie < chinese.EPOCH_1929_RD
      ? chinese.LOCATION_BEFORE_1929
      : chinese.LOCATION_SINCE_1929;
  }

  // Determine Julian day number (JDN) from Chinese calendar date
  private static calculateJdn(
    cycle: number,
    year: number,
    month: number,
    monthLeap: boolean,
    ...
    const midYear =
      Math.floor(this.getEpochRD() + J0000 + ((cycle - 1) * 60 + year - 0.5) * MEAN_TROPICAL_YEAR) -
      J0000;
    ...
    const priorNewMoon =
      month === d.getMonth() && monthLeap === d.isMonthLeap()
        ? p
        : this.chineseNewMoonOnOrAfter(1 + p);
  ...
  */
  ...
  private static chineseWinterSolsticeOnOrBefore(rataDie: number): number {
    const approx: number = estimatePriorSolarLongitude(
      Season.WINTER,
      this.midnightInChina(rataDie + 1),
    );

    return next(
      Math.floor(approx) - 1,
      (day: number) => Season.WINTER < solarLongitude(this.midnightInChina(1 + day)),
    );
  }
