...
export class HebrewObservationalCalendarDate extends YearMonthCalendarDate {
  ...
  public static calculateJdn(year: number, month: number, day: number): number {
    ...
    const start: number = HebrewCalendarDate.toJdn(year1, HebrewMonth.NISAN, 1);
    const gYear: number = GregorianCalendarDate.jdnToYear(start + 60);
  ...
  public static fromJdn(jdn: number): HebrewObservationalCalendarDate {
    ...
    const gYear: number = GregorianCalendarDate.jdnToYear(jdn);
    ...
    const year: number = HebrewCalendarDate.fromJdn(newYear2).getYear() + (month >= HebrewMonth.TISHRI ? 1 : 0);
  ...
  private static toNewYear(year: number): number {
    const jan1: number = GregorianCalendarDate.toJdn(year, 1, 1) - J0000;
