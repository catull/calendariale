...
export class PersianAstronomicalCalendarDate extends LeapCalendarDate {
  ...
  private static persianNewYearOnOrBefore(jdn: number): number {
    ...
    return next(Math.floor(approx) - 1, (day: number): boolean =>
      solarLongitude(PersianAstronomicalCalendarDate.midDayInTehran(day)) <= Season.SPRING + 2
    );
  }
  ...
  constructor(jdn: number, year: number, month: number, day: number) {
    super(jdn, year, month, day, PersianAstronomicalCalendarDate.isLeapYear(year));
  }
