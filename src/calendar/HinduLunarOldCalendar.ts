...
export class HinduLunarOldCalendar {
  ...
  public static isLeapYear(year: number): boolean {
    return (
      mod(year * ARYA_SOLAR_YEAR - ARYA_SOLAR_MONTH, ARYA_LUNAR_MONTH) >= 23902504679 / 1282400064
    );
  }
  ...
  private static calculateJdn(
    ...
    return (
      Math.ceil(
        hindu.EPOCH + lunarNewYear + ARYA_LUNAR_MONTH * temp + (day - 1) * ARYA_LUNAR_DAY - 0.75,
      ) + 0.5
    );
  }
}
