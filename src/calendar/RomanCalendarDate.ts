...
import { JulianCalendarDate, daysInMonth } from './JulianCalendarDate';
...
export class RomanCalendarDate extends YearMonthCalendarDate {
  ...
  public static toJdn (year: number, month: number, event: RomanEvent, count: number, leap: boolean): number {
    ...
    let jdn: number = JulianCalendarDate.toJdn (year, month, day) - count;

    if (leap) {
      jdn += 1;
    }

    if (!JulianCalendarDate.isLeapYear (year) ||
       month !== Month.MARCH ||
  ...
  public static validate (year: number, month: number, event: RomanEvent, count: number, leap: boolean): void {
    ...
    if (leap && (event !== RomanEvent.KALENDS || month !== 3 || count !== 6 || !JulianCalendarDate.isLeapYear (year))) {
  ...
  public static fromJdn (jdn: number): RomanCalendarDate {
    const date: JulianCalendarDate = JulianCalendarDate.fromJdn (jdn);
    ...
    } else if (month !== Month.FEBRUARY || !JulianCalendarDate.isLeapYear (year)) {
