...
export class GregorianCalendarDate extends LeapCalendarDate {
  ...
  public static toJdn(year: number, month: number, day: number): number {
    ...
    return gregorian.EPOCH - 1 + 365 * y1 +
      ...
      Math.floor((367 * month - 362) / 12 +
        (month <= 2 ? 0 : GregorianCalendarDate.isLeapYear(year) ? -1 : -2) + day);
  }
  ...
  public static fromJdn(jdn: number): GregorianCalendarDate {
    const jd0: number = Math.floor(jdn - 0.5) + 0.5;
    const year: number = GregorianCalendarDate.jdnToYear(jd0);
    const yearDay: number = jd0 - GregorianCalendarDate.toJdn(year, 1, 1);
    const leapAdj: number = jd0 < GregorianCalendarDate.toJdn(year, 3, 1) ? 0 : GregorianCalendarDate.isLeapYear(year) ? 1 : 2;
    const month: number = Math.floor(((yearDay + leapAdj) * 12 + 373) / 367);
    const day: number = jd0 - GregorianCalendarDate.toJdn(year, month, 1) + 1;
  ...
  constructor(jdn: number, year: number, month: number, day: number) {
    super(jdn, year, month, day, GregorianCalendarDate.isLeapYear(year));
  }
