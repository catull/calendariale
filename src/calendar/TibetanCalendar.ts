...
export class TibetanCalendar {
  ...
  public static fromJdn(jdn: number): TibetanDate {
    ...
    const year: number =
      day > day0 && month0 === 1 ? year0 - 1 : monthLeap && month0 === 12 ? year0 + 1 : year0;
    const dayLeap: boolean =
      jdn === TibetanCalendar.calculateJdn(year, month, monthLeap, day, true);
  ...
  public static isLeapMonth(year: number, month: number): boolean {
    return (
      month ===
      TibetanCalendar.fromJdn(TibetanCalendar.calculateJdn(year, month, true, 2, false)).getMonth()
    );
  }
