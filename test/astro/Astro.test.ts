...
import { WeekDay, babylonian, islamic, J1970 } from "../../src/Const";
...
const dates = [
  ...
  },
  {
    jdn: 2091164.5,
    rataDie: 369740,
    rise: { tee: 0.434271, h: 10, m: 25, s: 21 },
    set: { tee: -1, h: 0, m: 0, s: 0 },
  },
  {
    jdn: 2121509.5,
    rataDie: 400085,
  ...
  },
  {
    jdn: 2245580.5,
    rataDie: 524156,
    rise: { tee: 0.45219, h: 10, m: 51, s: 9 },
    set: { tee: -1, h: 0, m: 0, s: 0 },
  },
  {
    jdn: 2266100.5,
  ...
  },
  {
    jdn: 2431004.5,
    rataDie: 709580,
    rise: { tee: 0.555246, h: 13, m: 19, s: 33 },
    set: { tee: -1, h: 0, m: 0, s: 0 },
  },
  {
    jdn: 2448698.5,
  ...
  },
  {
    jdn: 2450138.5,
    rataDie: 728714,
    rise: { tee: 0.466211, h: 11, m: 11, s: 21 },
    set: { tee: -1, h: 0, m: 0, s: 0 },
  },
  {
    jdn: 2465737.5,
...
describe("astro spec", () => {
  ...
  it("should calculate a polynomial", () => {
    expect(
      poly(1, [-0.00002, 0.000297, 0.025184, -0.181133, 0.55304, -0.861938, 0.677066, -0.212591]),
    ).toBeCloseTo(-0.000095, 1e-6);

    expect(
      poly(50, [-0.00002, 0.000297, 0.025184, -0.181133, 0.55304, -0.861938, 0.677066, -0.212591]),
  ...
  it("should sort an array with binary search", () => {
    ...
    const predicate = (a0: number, b0: number): boolean =>
      Math.abs(fMinusY((a0 + b0) / 2, y1)) <= 1e-5;
  ...
  it("should calculate moon-rise for rata die", () => {
    dates.forEach(({ rataDie, rise }) =>
      expect(moonRise(rataDie, islamic.LOCATION_MECCA)).toBeCloseTo(rataDie + rise.tee, 0.00001),
    );
  });

  it("should calculate moon-set for rata die to be -1", () => {
    dates
      .filter(({ set }) => set.tee === -1)
      .forEach(({ rataDie }) =>
        // 369740, 524156, 709580, 728714
        expect(moonSet(rataDie, islamic.LOCATION_MECCA)).toBe(-1),
      );
  });
  ...
  it("should calculate moon-set for rata die", () => {
    dates
      .filter(({ set }) => set.tee !== -1)
      .forEach(({ rataDie, set }) =>
        expect(moonSet(rataDie, islamic.LOCATION_MECCA)).toBeCloseTo(rataDie + set.tee, 0.00001),
      );
  });
