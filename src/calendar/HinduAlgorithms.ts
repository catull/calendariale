...
function hinduAstroDateYear(tee: number): number {
  return Math.round(
    (tee - hindu.EPOCH_RD) / MEAN_SIDEREAL_YEAR - siderealSolarLongitude(tee) / 360,
  );
}
...
function hinduSine(theta: number): number {
  ...
  return (
    fraction * hinduSineTable(Math.ceil(entry)) + (1 - fraction) * hinduSineTable(Math.floor(entry))
  );
}
...
function hinduTropicalLongitude(jdn: number): number {
  ...
  const precession2: number =
    27 - Math.abs(108 * mod3((600 / 1577917828) * days - 0.25, -0.5, 0.5));
